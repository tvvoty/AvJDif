...
def isJapanese(text):
    lang_det_det = detect(text)
    print(f"The language is {lang_det_det}")

    langs_det_det = detect_langs(text)
    print(f"The lang probabilities are: {langs_det_det}")
    if lang_det_det == "ja":
        return True
    if lang_det_det == "en":
        return True
...
def find_avg_diff_of_folder_with_hashtable():
    for folder in os.listdir(main_folder):
        ...
        for file in glob.glob(f"{os.getcwd()}\\**\\**.**", recursive=True):
            ...
            print(file_path)
            try:
                with open(file_path, mode='r', encoding='utf-8') as sub_file:
                    sub_file_content = sub_file.read()
                    if isJapanese(sub_file_content):
                        parsed_text = wakati.parse(sub_file_content).split()
                        # print(parsed_text)
